export default function decorate(block) {
  ...
  function parseStoreHours(hoursText) {
    ...
    // Expected format: "Store Hours: Day(s) Hours MON-FRI: 7:00am-7:00pm SAT: 7:00am-6:00pm SUN: 8:00am-5:00pm Christmas Eve: 8:00am-4:00pm Christmas: Closed"
    const hours = [];
    
    // Remove "Store Hours: Day(s) Hours " prefix
    const cleanText = hoursText.replace(/^Store Hours:\s*Day\(s\)\s*Hours\s*/i, '');
    
    // Parse each day/time segment
    const patterns = [
  ...
  function parseAddress(addressText) {
    ...
    // Remove phone and clean address
    let address = addressText.replace(/\d{3}[.\-]?\d{3}[.\-]?\d{4}/, '').trim();
    
    // Split address into lines
    const parts = address.split(/(?=[A-Z][a-z]+,)/); // Split before "Nashville,"
    const lines = [];
    
    if (parts.length > 1) {
      lines.push(parts[0].trim()); // "406 Gallatin Ave"
      lines.push(parts[1].trim()); // "Nashville, TN"
    } else {
  ...
  const serviceData = {
    ...
    alignment: {
      ...
      storeMap: '',
      storeHoursRaw: 'Store Hours: Day(s) Hours MON-FRI: 7:00am-7:00pm SAT: 7:00am-6:00pm SUN: 8:00am-5:00pm Christmas Eve: 8:00am-4:00pm Christmas: Closed',
      storeDetailsLink: '#',
  ...
  function createServiceDetail(serviceName, data) {
    ...
    // Store details grid (address + map)
    ...
    // Close button handler - hide detail view and show top nav and tab nav
  ...
  // Service item click handler - hide top nav when opening service detail
  ...
  crossSectionSelect.addEventListener('change', function() {
