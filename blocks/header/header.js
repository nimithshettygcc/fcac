...
export default async function decorate(block) {
  ...
  sections.forEach((section) => {
    ...
    if (utilitySections.includes(section)) {
      ...
      let labelText = 'Sign In';
      if (firstLink) {
        labelText = firstLink.textContent.trim();
      } else if (isBlog) {
        labelText = 'Blog';
      }
    ...
    if (menuTitle && titleToMap) {
      ...
      if (isMega) {
        ...
        if (menuTitle === 'Tires') {
          ...
          const wrapper = section.querySelector('.sub-nav-content.tires') || section.querySelector('.default-content-wrapper') || section;
            ...
            if (!inRightCol) {
              ...
              if (el.tagName === 'H4') {
                ...
                el.querySelectorAll('li').forEach((liSource) => {
                  ...
                  const nameText = fullText.split(' ')[0];
                  const taglineText = fullText.substring(nameText.length).trim();
          ...
          if (sureDriveLinkElement && brandsGrid) {
            ...
            for (let i = 0; i < brandsGrid.children.length; i += 1) {
            ...
            if (!targetColumn && brandsGrid.children.length > 1) {
              [, targetColumn] = brandsGrid.children;
            }
            if (!targetColumn) {
          ...
          elements.forEach((el) => {
            ...
            if (elementTextLower.includes('schedule') || elementTextLower.includes('save time') || el.querySelector('.button-container')) {
              const clone = el.cloneNode(true);
          ...
          if (cloneHeadings[0] && cloneHeadings[0].textContent.includes(menuTitle)) {
            cloneHeadings[0].remove();
          }
